%% Run the non-parametric perumutation test on task-combined
% beta images using SnPM package 
...
secondLevelFolder = [pwd,'/CombinedGLM_Feedback_3mm_Multimodel_Searchlight_9mm_1ss_Level2'];
...
for RDM = 1:length(modelNames)
        ...
        cur_folder = sprintf('%s/%s/%s/perm_test',secondLevelFolder, method_str,modelNames{RDM});
        ...
        matlabbatch{1}.spm.tools.snpm.des.OneSampT.masking.em = {'/gpfs/data/ofeldman/alamba1/adult_social_risk/Final_pipeline_scripts/Final_RSA_analysis_scripts/Subject_Masks/group_whole_brain_binary.nii'};
