 function [pred_RDM,conditionVec] = construct_pred_RDMs_RSA_rpe_signed_choice(subNum,condition,scriptPath,RDM_roi)
...
% get subject data
data = readtable("adult_behavData_1ca2lr.csv");
...
delta_z_shift = zeros(height(sub_dat),1);

% shift by one trial previous
for trial = 1:height(sub_dat)
   
    
    pNum = sub_dat.playerNum(trial);
    binned_trial = sub_dat.binned_trial_num(trial);
    
    if binned_trial > 1
        
        prev_trial = binned_trial-1; 
        p_dat = sub_dat(sub_dat.playerNum == pNum,:);
        prev_delta = table2array(p_dat(p_dat.binned_trial_num == prev_trial,15));
        delta_z_shift(trial) = prev_delta; 
        
    else 
        
        delta_z_shift(trial) = NaN; 
    
    end 
    
end 

sub_dat.delta_z_shift = delta_z_shift; 
    
% change the threshold here to check if binning artifact
neg_rpes = find(sub_dat.delta_z_shift(:) < 0);
pos_rpes = find(sub_dat.delta_z_shift(:) >= 0); 
