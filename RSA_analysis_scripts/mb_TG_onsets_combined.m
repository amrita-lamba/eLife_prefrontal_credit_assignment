...
function mb_TG_onsets_combined(subNum,counterBalance,scriptPath)
...
sub_idx = find(mlFile.subjNum == subNum & mlFile.condition == 1);
...
run1_faceOnsets = run1_tgData.faceOnset;
run1_faceDurations = run1_tgData.trialRT;
run2_faceOnsets = run2_tgData.faceOnset + 492;
run2_faceDurations = run2_tgData.trialRT;

% pull feedback onsets and durations 
run1_feedbackOnsets = run1_tgData.feedbackOnset; 
run2_feedbackOnsets = run2_tgData.feedbackOnset + 492; 
