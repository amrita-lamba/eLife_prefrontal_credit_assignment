function [pred_RDM,conditionVec] = construct_pred_RDMs_choice(subNum,condition,scriptPath)
...
for playerNum = 1:4
    ...
    for trial = 1:15
        ...
        conditionVec(cnt,1) = find(contains(SPM.xX.name,trial_str));
...
for playerNum = 1:4
    ...
    for trial = 1:15
        ...
        if cur_choice ~= -1 
            ...
            if trial > 1
            % update vector
            qval_vec(cnt) = p_qval_val(trial-1); % shift by one trial (link BOLD image with qval on PREVIOUS trial)
            else
