function [u_hat,resMS,Sw_hat,beta_hat,shrinkage,trRR]=noiseNormalizeBeta_2015(Y,SPM,varargin)
% function
% [u_hat,Sw_hat,resMS,beta_hat]=rsa_noiseNormalizeBeta(Y,SPM,varargin)
% Estimates beta coefficiencts beta_hat and residuals from raw time series
% Y
% Estimates the true activity patterns u_hat by applying noise
% normalization to beta_hat
% INPUT:
%    Y        raw timeseries, T by P
%    SPM:     SPM structure
% OPTIONS:
%   'normmode':   'overall': Does the multivariate noise normalisation
%   overall (default)
%                 'runwise': Does the multivariate noise normalisation by
%                 run
%   'shrinkage':  Shrinkage coefficient.
%                 0: No regularisation
%                 1: Using only the diagonal - i.e. univariate noise
%                 normalisation
%                 By default the shrinkage coeffcient is determined using
%                 the Ledoit-Wolf method.
% OUTPUT:
%    u_hat:       estimated true activity patterns (beta_hat after
%    multivariate noise normalization),
%    resMS:       residual mean-square - diagonal of the Var-cov matrix, 1
%    by P
%    Sw_hat:      overall voxel error variance-covariance matrix (PxP),
...
%%% Get partions: For each run (1:K), find the time points (T) and
%%% regressors (K+Q) that belong to the run
partT = nan(T,1);
partQ = nan(Q,1);
...
%        ***** AVI EDIT ****
%         ORIGINAL NOISE NORMALIZATION FUNCTION
...
%       NOISE NORMALIZATION FROM THE DECODING TOOLBOX v.3.994
        Sw_reg = covshrink_lw(res);
        
        % Postmultiply by the inverse square root of the
        % estimated matrix
        [V,L]=eig(Sw_reg);
        l=diag(L);
...
% If prewhiting was perfect, then the trace(sig*sig) = P and the effective
% number of voxels
% would be P. However, because we need to regularise our estimate, the
% residual
% spatial covariance matrix is somwhat correlated. For variance estimation
% on the distances,
